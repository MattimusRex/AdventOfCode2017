...
namespace Day21_2
{
    class Program
    {
        ...
        static private bool ArraysEqual(char[][] arr, char[][] checkArr)
        {
            if (arr.Length != checkArr.Length)
            {
                Console.WriteLine("arrays not same length");
                return false;
            }
            for (int i = 0; i < arr.Length; i++)
            {
                if (arr[i].Length != checkArr[i].Length)
                {
                    Console.WriteLine("arrays[i] not same length.  i: " + i);
                    return false;
                }
                for (int j = 0; j < arr[i].Length; j++)
                {
                    if (arr[i][j] != checkArr[i][j])
                    {
                        ...
                        Console.WriteLine("checkArr[" + i + "][" + j + "] = " + checkArr[i][j]);
                        return false;
                    }
                }
            }
        ...
        static private void RunTests()
        {
            ...
            TestEnhance(testStart);
        }

        static private void TestEnhance(string testString)
        {
            char[][] arr = StringToArray(testString);
